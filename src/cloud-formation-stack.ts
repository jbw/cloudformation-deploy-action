...
export class CloudFormationStack {
  ...
  public async deploy(options?: { waitFor?: boolean }): Promise<CloudFormationStackResponse> {
    const stack = await this.getStack(this.stackOptions.name);
    return !stack ? this.create(options) : this.update(stack, options);
  }

  private async create(deployOptions?: options): Promise<CloudFormationStackResponse> {
